...
\clearpage
\section{Robot Logic}
The largest obstacle we overcame was understanding the difference between theoretical computer science and physical robotic computing.  On the white board and in the computer, issuing commands such as \textit{turnRight(35\textdegree)} then \textit{goForward(10 inches)} would move the robot 10 inches at 35\textdegree from the current location.

In actuality, this is never the case, especially when working with a remote control vehicle.  Our logic must perform numerous checks to confirm that a command produced the desired result.
